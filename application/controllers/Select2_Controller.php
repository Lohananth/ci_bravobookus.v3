...
class Select2_Controller extends CI_Controller {
  ...
  function __construct() {
      ...
      $this->load->database();
      $this->perPage = 5;
      $this->load->model('Select2_Model','select2_model',True); 
  ...
  public function generate_report(){
    ...
    WHERE vs.status=1";
    $data['vschedule_list']=$this->select2_model->get_by_sql($vSQL);
    $this->load->view('pdf_view',$data);
  }

  public function index(){    
    $data=array();      
    $data['settings']=$this->select2_model->get_by_sql("SELECT * FROM settings");
   // Count Vehicles
    $data['vehicles_count']=$this->select2_model->get_by_sql("SELECT count(v_id) as vehicles_count FROM tbl_vehicle");
    $uid=$this->session->userdata('uid');
    $data['sidebar_menu']=$this->select2_model->get_by_sql("SELECT * FROM tbl_controllers");
    $data['user_groups']=$this->select2_model->get_by_sql("SELECT * FROM user_groups WHERE id_group=$gro_id");
    ...
    //load the view
    $this->load->view('admin/v_admin_select2', $data);
  }
  ...
  public function list_schedules(){      
    ...
    $gro_id=$this->session->userdata('gro_id');
     // $data['sidebar_menu']=$this->select2_model->get_by_sql("SELECT * FROM tbl_controllers where uid=$uid");
    $data['sidebar_menu']=$this->select2_model->get_by_sql("SELECT * FROM tbl_controllers");
    $data['user_groups']=$this->select2_model->get_by_sql("SELECT * FROM user_groups WHERE id_group=$gro_id");
    $data['form_title']=$this->replaceAll($this->uri->segment(1));
    $data['panel_title']=$this->uri->segment(1);
    $data['head']='admin/head/v_head_table';
    $data['footer']='admin/footer/v_footer_select2';
    $data['sidebar']='admin/inc/v_sidebar';
    $data['sidebar_right']='admin/inc/v_sidebar_right';
    $data['header']='admin/inc/v_header';   
    $data['origins']=$this->select2_model->get_by_sql("SELECT * FROM tbl_origin");
    $data['vehicles']=$this->select2_model->get_by_sql("SELECT * FROM tbl_vehicle");
    $data['dptimes']=$this->select2_model->get_by_sql("SELECT * FROM tbl_departure_time");  
    $vSQL = "SELECT vs.id AS id, ori.origin AS origin, ori1.origin AS destination,
        vh.vehicle_name AS vehicle_name,dpt.departure_time AS departure_time,
        vs.travel_duration AS travel_duration, vs.local_price AS local_price,
        vs.foreigner_price AS foreigner_price, vs.`status` AS `status`
        ...
        WHERE vs.status=1";
    $data['vschedule_list']=$this->select2_model->get_by_sql($vSQL);
    $data['main_content']='admin/test/v_list';
          //load the view
  ...
  public function list_schedules_blocked(){      
     ...
     // $data['sidebar_menu']=$this->select2_model->get_by_sql("SELECT * FROM tbl_controllers where uid=$uid");
      ...
      $vs_block="SELECT vs.id AS id, ori.origin AS origin, ori1.origin AS destination,
                          ...
                          JOIN tbl_departure_time AS dpt ON (dpt.id = vs.departure_time)
                          WHERE vs.status=0";
      $data['vschedule_list']=$this->select2_model->get_by_sql($vs_block);
...
//Ajax Operation Crud
  public function Save_Vechicle_Schedule(){
    $data_in = array(
        'origin' => $this->input->post('origin'),
        'destination' => $this->input->post('destination'),
        'v_id' => $this->input->post('v_id'),
        'local_price' => $this->input->post('local_price'),
        'foreigner_price' => $this->input->post('foreigner_price'),
        'departure_time' => $this->input->post('departure_time'),
        'travel_duration' => $this->input->post('travel_duration'),
        'status'  => $this->input->post('status_hide'),
      );
    $vs_schedule = $this->select2_model->Save_Vechicle_Schedule($data_in); 
    if($vs_schedule)
  ...
  public function get_Vechicle_Schedule(){
    //Get Cate_id from Ajax
    $form = ''; 
    $data = array();
    $data['origins']=$this->select2_model->get_by_sql("SELECT * FROM tbl_origin");
    $data['vehicles']=$this->select2_model->get_by_sql("SELECT * FROM tbl_vehicle");
    ...
    $vSQL = "SELECT vs.id AS id, ori.origin AS origin, ori1.origin AS destination,
        ...
        JOIN tbl_vehicle AS vh ON (vh.v_id = vs.v_id)
        JOIN tbl_departure_time AS dpt ON (dpt.id = vs.departure_time)
        WHERE vs.status=1 AND vs.id=$id";
    $data['vschedule_list']=$this->select2_model->get_by_sql($vSQL);
    // var_dump($data['dptimes']);exit();
    // var_dump($single_dptime = $data['vschedule_list'][0]['departure_time']);exit();
    $single_ori = $data['vschedule_list'][0]['origin'];
    $single_dest = $data['vschedule_list'][0]['destination'];
    $single_vs_name = $data['vschedule_list'][0]['vehicle_name'];
    $single_dptime = $data['vschedule_list'][0]['departure_time'];
    $single_status = $data['vschedule_list'][0]['status'];
    $form .='<div class="row">';
    $form .='<div class="form-group">';
    $form .='<div class="col-md-6">';
    $form .='<label for="origin">Origin(From)<span><b>*</b></span></label>';
    $form .='<input type="hidden" name="id" value="'.$data['vschedule_list'][0]['id'].'">';
    $form .='<select data-plugin-selectTwo name="origin" id="origin" class="form-control">
            <optgroup label="Select Original">';
    foreach ($data['origins'] as $ori) {
      if($ori['origin']==$single_ori){
        $form .='<option value="'.$ori['id'].'" selected="selected">'.$ori['origin'].'</option>';
      } else {
        $form .='<option value="'.$ori['id'].'">'.$ori['origin'].'</option>';    
      }
    }
    $form .='</select>';
    $form .='</div>';
    ...
    $form .='<select data-plugin-selectTwo name="destination" id="destination" class="form-control">
            <optgroup label="Select Destinationn">';
    foreach ($data['origins'] as $dest) {
      if($dest['origin']==$single_dest){
        $form .='<option value="'.$dest['id'].'" selected="selected">'.$dest['origin'].'</option>';
      } else {
        $form .='<option value="'.$dest['id'].'">'.$dest['origin'].'</option>';    
      }
    }
    $form .='</select>';
    $form .='</div>';
    ...
    $form .='<label for="v_id">Vehicle Name<span><b>*</b></span></label>';
    $form .='<select data-plugin-selectTwo name="v_id" id="v_id" class="form-control">
            <optgroup label="Select Destinationn">';
    foreach ($data['vehicles'] as $vs) {
      if($vs['vehicle_name']==$single_vs_name){
        $form .='<option value="'.$vs['v_id'].'" selected="selected">'.$vs['vehicle_name'].'</option>';
      } else {
        $form .='<option value="'.$vs['v_id'].'">'.$vs['vehicle_name'].'</option>';    
      }
    }
    $form .='</select>';
    $form .='</div>';
    ...
    $form .='<input type="text" name="local_price" id="local_price" class="form-control" value="'.$data['vschedule_list'][0]['local_price'].'">';
    $form .='</div>';
    $form .='<div class="col-md-4">';
    $form .='<label for="foreigner_price">Foreign Price<span><b>*</b></span></label>';
    $form .='<input type="text" name="foreigner_price" id="foreigner_price" class="form-control" value="'.$data['vschedule_list'][0]['foreigner_price'].'">';
    $form .='</div>';
    $form .='<div class="col-md-4">';
    $form .='<label for="departure_time">Departure Time<span><b>*</b></span></label>';
    $form .='<select data-plugin-selectTwo name="v_id" id="v_id" class="form-control">
            <optgroup label="Select Destinationn">';
    foreach ($data['dptimes'] as $dptime) {
      if($dptime['departure_time']==$single_dptime){
        $form .='<option value="'.$dptime['id'].'" selected="selected">'.$dptime['departure_time'].'</option>';
      } else {
        $form .='<option value="'.$dptime['id'].'">'.$dptime['departure_time'].'</option>';    
      }
    }
    $form .='</select>';
    $form .='</div>';
    ...
    $form .='<input type="text" name="travel_duration" id="travel_duration" class="form-control" value="'.$data['vschedule_list'][0]['travel_duration'].'">';
    $form .='</div>';
    $form .='<div class="col-md-6 checkbox" style="padding-top: 0px; margin-top:15px;">';
    $form .='<label for="origin">Active/Block&nbsp;</label>';
    if($single_status==1){
      $form .='<input type="checkbox" name="status" id="status" class="status" data-toggle="toggle" data-on="Active" data-off="Block" data-onstyle="success" data-offstyle="danger" checked />';
      $form .='<input type="hidden" name="status_hide" id="status_hide_upd" value="1" class="status_hide_upd" />';
    } else {
      $form .='<input type="checkbox" name="status" id="status" class="status" data-toggle="toggle" data-on="Active" data-off="Block" data-onstyle="success" data-offstyle="danger" />';
      $form .='<input type="hidden" name="status_hide" id="status_hide_upd" value="0" class="status_hide_upd" />';
    }
    $form .='</div>';
      // End foreach
  ...
  public function Update_Vechicle_Schedule(){
    ...
    $update_cate = $this->select2_model->update_category_query($v_id,$data_update);
    if($update_cate)
      echo "1";
    else
      echo "0";
...
//End Ajax Operation
  ...
  public function editForm($id=''){      
     ...
     // $data['sidebar_menu']=$this->select2_model->get_by_sql("SELECT * FROM tbl_controllers where uid=$uid");
      ...
      $data['data_vehicles'] = $this->select2_model->get_by_sql($vSQL);
  ...
  public function viewForm(){      
    ...
    $vsview = $this->select2_model->getView($vs_id);
    if (count($vsview)>0){
  ...
  public function get_block_Schedule(){
    $form = ''; 
    $vs_id = $this->input->post('vs_id');
    $vslist = $this->select2_model->get_blocked_schedule_query($vs_id);
    if (count($vslist)>0){
      foreach ($vslist as $vs){
        ...
        $form .='<center> <h4>Do you want to Blocked Vehicle Schedule ?<h4><h3 style="color:red;">'.$vs->id.' '.$vs->vehicle_name. '</h3></center>';
      }  
          // End foreach
      echo json_encode($form);    
    }    
  ...
  public function viewBlock(){      
    $form = ''; 
    $vs_id = $this->input->post('vs_id');
    $vsview = $this->select2_model->getBlock($vs_id);
    if (count($vsview)>0){
    foreach ($vsview as $vs){
      $form .='<div class="row">';
      $form .='<div class="form-group">';
  ...
  public function update_block_Schedule(){
    ...
    $update_cate = $this->select2_model->update_blocked_schedule_query($vs_id,$data_update);
    if($update_cate)
      echo "1";
    else
      echo "0";
  ...
  public function get_active_Schedule(){
    $form = ''; 
    $vs_id = $this->input->post('vs_id');
    $vslist = $this->select2_model->get_active_schedule_query($vs_id);
    if (count($vslist)>0){
      foreach ($vslist as $vs){
        $form .=' <input type="hidden" name="vs_id" id="vs_id" value="'.$vs->id.'">';
        $form .=' <input type="hidden" name="status" id="status" value="1">';
        $form .=' <center> <h4>Do you want to Active Vehicle ?<h4><h3 style="color:green;">'.$vs->id.' '.$vs->vehicle_name. '</h3></center>';
      }  
          // End foreach
      echo json_encode($form);    
    }    
  ...
  public function update_active_Schedule(){
    ...
    $update_cate = $this->select2_model->update_active_schedule_query($vs_id,$data_update);
    if($update_cate)
      echo "1";
    else
      echo "0";

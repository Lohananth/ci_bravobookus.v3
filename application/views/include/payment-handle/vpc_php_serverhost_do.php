...
$SECURE_SECRET = "AD68AA08F19A9EBAC53A42DD732A62F0";

// add the start of the vpcURL querystring parameters
$vpcURL = trim(Decrypt('mykey', $_POST["virtualPaymentClientURL"])) . "?";
...
foreach($_POST as $key => $value) {
    ...
    if (strlen(trim(Decrypt('mykey', $value))) > 0) {
        
        // this ensures the first paramter of the URL is preceded by the '?' char
        if ($appendAmp == 0) {
            $vpcURL .= urlencode($key) . '=' . urlencode(trim(Decrypt('mykey', $value)));
            $appendAmp = 1;
        } else {
            $vpcURL .= '&' . urlencode($key) . "=" . urlencode(trim(Decrypt('mykey', $value)));
        }
        $md5HashData .= trim(Decrypt('mykey', $value));
    }
}

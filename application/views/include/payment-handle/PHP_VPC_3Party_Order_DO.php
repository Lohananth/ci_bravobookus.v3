...
$title  = trim(Decrypt('mykey', $_POST["Title"]));
...
foreach($_POST as $key => $value) {
	$tmp_value = trim(Decrypt('mykey', $value));
	if (strlen($tmp_value) > 0) {
		$conn->addDigitalOrderField($key, $tmp_value);
	}
}
...
$vpcURL = $conn->getDigitalOrder($vpcURL); //echo $vpcURL; exit();
